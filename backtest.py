...
def run_backtest(config_file: Optional[str] = None, logger: Optional[logging.Logger] = None):
    ...
    try:
        ...
        # IMPROVEMENT: NaN 비율 체크(선택 적용)
        strategy = results[0] if results else None
        if strategy is not None:
            pf_values = strategy.get_portfolio_values()
            
            if pf_values:
                nan_count = sum(pd.isna(v) for v in pf_values)
                nan_ratio = nan_count / len(pf_values)
                if nan_ratio > 0.3:
                    log.warning(f"More than 30% of portfolio values are NaN! (NaN ratio={nan_ratio:.2f})")
...
def create_portfolio_value_chart(strategy, initial_value, final_value, timestamp, logger=None):
    ...
    log = logger or logging.getLogger('backtest')
    try:
        dates = strategy.get_portfolio_dates()
        values = strategy.get_portfolio_values()
...
def create_annual_returns_chart(strategy, timestamp, logger=None):
    ...
    try:
        ...
        for i, bar in enumerate(bars):
            # 막대 색상 관련 원본 로직 유지(blue/red) - 필요에 따라 수정 가능
...
def create_monthly_returns_heatmap(strategy, timestamp, logger=None):
    ...
    try:
        ...
        if not dates or not values or len(dates) != len(values):
            log.warning("Inconsistent data for monthly returns heatmap")
            return
        ...
        monthly_returns = df['Value'].resample('ME').last().pct_change(fill_method=None) * 100
...
def create_drawdown_chart(strategy, timestamp, logger=None):
    ...
    log = logger or logging.getLogger('backtest')
    try:
        values = np.array(strategy.get_portfolio_values())
        dates = strategy.get_portfolio_dates()
        ...
        max_dd_date = dates[max_dd_idx].strftime('%Y-%m-%d') if len(dates) > 0 else ''
        ax.axhline(y=max_dd, color='r', linestyle='--', alpha=0.7)
        ax.text(dates[len(dates)//2], max_dd * 1.1 if not np.isnan(max_dd) else 0, 
                f'Max Drawdown: {max_dd:.2f}% ({max_dd_date})', ha='center', color='r')
        plt.tight_layout()
...
def save_results_to_csv(strategy, timestamp, logger=None):
    ...
    try:
        portfolio_df = pd.DataFrame({
            'Date': strategy.get_portfolio_dates(),
            'Value': strategy.get_portfolio_values()
        })
        ...
        if daily_returns:
            daily_df = pd.DataFrame({
                'Date': strategy.get_portfolio_dates()[1:],  # 첫 날은 수익률 없음
