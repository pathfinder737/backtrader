...
class MonthlyRebalanceStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 주간 이동평균 사용 시 주간 데이터 파일 로드
        self.weekly_data = None
        if self.p.ma_type == 'weekly':
            try:
                weekly_file = os.path.join(config.config.DATA_DIR, f"{self.p.touchstone}_weekly.csv")
                if os.path.exists(weekly_file):
                    # 인덱스 컬럼을 'Date'로 지정하여 로드
                    self.weekly_data = pd.read_csv(weekly_file, index_col='Date', parse_dates=True)
                    self.logger.info(f"Loaded weekly data for {self.p.touchstone} from {weekly_file}")
                    
                    # 필요한 WMA 컬럼 확인
                    wma_cols = [f'WMA_{period}' for period in self.p.wma_periods]
                    missing_cols = [col for col in wma_cols if col not in self.weekly_data.columns]
                    if missing_cols:
                        self.logger.warning(f"Missing weekly MA columns: {missing_cols}")
                else:
                    self.logger.warning(f"Weekly data file not found: {weekly_file}")
            except Exception as e:
                self.logger.error(f"Error loading weekly data: {e}")
                import traceback
    ...
    def _determine_allocation_weekly(self, current_date, current_price) -> Dict[str, float]:
        ...
        별도로 로드된 주간 데이터에서 WMA 값을 찾습니다.
        """
        if self.weekly_data is None:
            self.logger.warning(f"[{current_date}] Weekly data not available. Using base allocation.")
            self.market_regime = "Neutral"
            return self.p.asset_allocation
        ...
        try:
            # 현재 날짜 이전의 가장 가까운 주간 데이터 찾기
            valid_dates = self.weekly_data.index[self.weekly_data.index <= current_date_ts]
            
            if len(valid_dates) == 0:
                self.logger.warning(f"No weekly data available before {current_date_ts}")
                self.market_regime = "Neutral"
                return self.p.asset_allocation
                
            latest_date = valid_dates[-1]
            latest_row = self.weekly_data.loc[latest_date]
            
            for period in self.p.wma_periods:
                wma_col = f'WMA_{period}'
                if wma_col in latest_row and not pd.isna(latest_row[wma_col]):
                    wma_values[period] = latest_row[wma_col]
                else:
                    self.logger.warning(f"{wma_col} not available in weekly data for date {latest_date}")
                    self.market_regime = "Neutral"
                    return self.p.asset_allocation
            ...
            # 예외 처리 강화
            self.logger.error(f"Error in weekly allocation determination: {e}")
            import traceback
            self.logger.error(traceback.format_exc())
            self.market_regime = "Neutral"
            return self.p.asset_allocation
